...
# The below code reproduces Figure 3 from CHR 2025 contribution, "It takes a village to write a book: mapping anonymous contributions in Stephen Langton's Quaestiones Theologiae". You can use it to:
...
# If you want to experiment with different parameters, run stylo(corpus.dir = "corpus_Langton_by_stemma_processed_masked") and use the GUI. Note that due to corpus masking, the max available MFW is 200 (would you like to test longer wordlists or other features, please email me at j.maliszewski@uw.edu.pl). Also, it is necessary to start at rank 2, since the most common token in the corpus is the MASKEDTOKEN.
